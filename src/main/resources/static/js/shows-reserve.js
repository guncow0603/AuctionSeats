...
function initShowsReservePage() {
    ...
    var scheduleId, showsId; // 회차 id, 공연 id.
    ...
    initAuctionSeats(scheduleId, showsId);
    ...
    function initRequiredData() {
        initQueryParams();
        initZoneGradeInfo(showsId);
    }

    // 좌석등급정보 동기로 불러옴
    function initZoneGradeInfo(showsId) {
        $.ajax({
            url: `${getUrl()}/api/v1/shows/${showsId}/seats`,
            type: "GET",
            async: false,
            headers: {
                "Authorization": token
    ...
    function initAuctionSeats(scheduleId, showsId) {
        $.ajax({
            url: `${getUrl()}/api/v1/shows/${showsId}/auction-seats?scheduleId=${scheduleId}`,
            type: "GET",
            headers: {
                "Authorization": token
            },
    ...
    function initReservedSeats(scheduleId) {
        $.ajax({
            url: `${getUrl()}/api/v1/shows/${scheduleId}/reserved-seats`,
            type: "GET",
            headers: {
                "Authorization": token
            },
