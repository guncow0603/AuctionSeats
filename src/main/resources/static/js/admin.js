...
function addRowToTable(zoneName, seatsCount) {
    const newRow = $('<tr>').append(
        $('<td>').text(zoneName),
        $('<td>').text(seatsCount),
        $('<td>').append($('<button>').text('제거').addClass('remove-zone').attr('onclick', 'removeZone(this)'))
    );
...
function removeZone(button) {
    ...
    const zoneName = row.find('td:first').text();

    // zones 배열에서 해당 구역을 제거합니다.
    zones = zones.filter(zone => zone.zoneName !== zoneName);
...
function submitPlace() {
    ...
    const zoneInfos = zones.map(zone => ({
        name: zone.zoneName,
